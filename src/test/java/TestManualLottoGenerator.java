...
public class TestManualLottoGenerator {
	...
	void 반환된_로또는_정해진_갯수와_일치한_수를_가져야_한다() {
		...
		Lotto lotto = generator.issueLotto(List.of(1, 2, 3, 4, 5, 6));
	...
	void 수동으로_생성된_로또번호는_정해진_범위의_수이다() {
		...
		assertThatThrownBy(() -> generator.issueLotto(List.of(1, 2, 3, 0, 5, 6)))
			.isInstanceOf(RuntimeException.class)
			.hasMessage("로또 번호는 "+LottoNumber.MIN_NUMBER+"~"+LottoNumber.MAX_NUMBER+"까지의 숫자여야 한다.");

		assertThatThrownBy(() -> generator.issueLotto(List.of(1, 2, 3, 46, 5, 6)))
			.isInstanceOf(RuntimeException.class)
			.hasMessage("로또 번호는 "+LottoNumber.MIN_NUMBER+"~"+LottoNumber.MAX_NUMBER+"까지의 숫자여야 한다.");
	}
	...
	void 수동으로_생성된_로또번호는_중복되는_수가_없어야_한다() {
		...
		assertThatThrownBy(() -> generator.issueLotto(List.of(1, 2, 3, 3, 5, 6)))
			.isInstanceOf(RuntimeException.class)
	...
	void 수동으로_생성된_로또번호는_정해진_갯수와_일치해야한다() {
		ManualLottoGenerator generator = new ManualLottoGenerator();
		assertThatThrownBy(() -> generator.issueLotto(List.of(1, 2, 5, 6, 7)))
			.isInstanceOf(RuntimeException.class)
			.hasMessage("로또 번호 개수가 "+LottoNumber.LOTTO_SIZE+"개가 아닙니다.");

		assertThatThrownBy(() -> generator.issueLotto(List.of(1, 2, 3, 5, 6, 7, 8)))
			.isInstanceOf(RuntimeException.class)
			.hasMessage("로또 번호 개수가 "+LottoNumber.LOTTO_SIZE+"개가 아닙니다.");
	}
}
